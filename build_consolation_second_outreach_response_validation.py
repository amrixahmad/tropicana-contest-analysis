...
OUTPUT_FILE = Path('outputs/second_outreach_response_validation.xlsx')
...
OUTREACH_STATUS_COL = '2nd outreach status'
OUTREACH_DATE_COL = '2nd_outreach_date'
...
def build_boundary_metadata(message, match_type):
    ...
    return {
        ...
        'outreach_message_from_name': clean_text(sender.get('name', '')),
        'outreach_message_from_id': clean_text(sender.get('id', '')),
        'outreach_message_text_length': len(message_text),
        'outreach_boundary_match_type': match_type,
    }


def fetch_messages_until_outreach(token, conversation_id, second_outreach_date):
    messages = []
    next_url = ''
    params = {'fields': MESSAGE_FIELDS, 'limit': 100}
    target_date = parse_date_only(second_outreach_date)
    seen_winner_side_message = False
    ...
    while True:
        ...
        for message in batch:
            ...
            created_at = parse_timestamp(message.get('created_time', ''))
            created_date = created_at.date() if created_at is not None else None
            is_page_message = is_page_sender(sender_name, sender_id)
            has_second_outreach = message_contains_second_outreach(message_text)

            if is_page_message and has_second_outreach:
                return messages, build_boundary_metadata(message, 'contains_second_outreach_fragment')

            if not is_page_message:
                seen_winner_side_message = True

            if (
                is_page_message
                and seen_winner_side_message
                and target_date is not None
                and created_date is not None
                and created_date >= target_date
            ):
                return messages, build_boundary_metadata(message, 'page_message_on_or_after_second_outreach_date')
...
def main():
    ...
    if OUTREACH_DATE_COL not in outreach.columns:
        raise KeyError(f'Expected column not found in outreach file: {OUTREACH_DATE_COL}')
    ...
    replied = outreach[
        outreach[OUTREACH_DATE_COL].ne('') & outreach[OUTREACH_STATUS_COL].eq('Replied')
    ].copy()
    ...
    for index, winner in enumerate(merged.to_dict(orient='records'), start=1):
        ...
        try:
            ...
            response_messages, outreach_boundary = fetch_messages_until_outreach(
                token,
                winner['conversation_id'],
                winner.get(OUTREACH_DATE_COL, ''),
            )
            if not outreach_boundary:
                raise RuntimeError('outreach boundary not found in message history')
            ...
            summary_rows.append({
                ...
                '2nd_outreach_date': winner.get(OUTREACH_DATE_COL, ''),
                '2nd_outreach_status': winner.get(OUTREACH_STATUS_COL, ''),
                'conversation_id': winner.get('conversation_id', ''),
                'conversation_updated_time': winner.get('conversation_updated_time', ''),
                'outreach_boundary_match_type': outreach_boundary.get('outreach_boundary_match_type', ''),
